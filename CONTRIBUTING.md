...
uv run pytest tests/ -q    # 373 tests, under 3 s
```

No system dependencies are needed for the base package or any of
the readers. `numba` and the C++ extension are pulled in by
`--all-extras`.
...
├── tests/              # 24 test modules, fixtures under tests/data/
...
`docs/ARCHITECTURE.md` has a per-module table if you're hunting
for where a feature lives.

## Development workflow

### Tests

pytest. The full suite has to pass before a PR can land:
...
`ruff format` is the formatter, not `black`. Line length is 100,
target version is `py311`.
...
- Each commit lands one cohesive change. Prefer many small commits
  over one big one. `git log --oneline` should read like a plan.
- Don't skip pre-commit hooks (`--no-verify`) or signing
  (`--no-gpg-sign`).
- When a perf change needs a rewrite, the perf commit IS the
  rewrite. No separate "refactor + perf" pair.
...
rinexpy is local-only. There's no PyPI publish step. A "release"
is a tagged commit on `main` plus a built wheel under `dist/`.
...
6. Build the wheel and sdist so users cloning at the tag can
   install without rebuilding:
   ```sh
   uv build --wheel --sdist
   ```
   Artifacts land under `dist/`. Attach them to the GitHub Release
