...
- **PPP driver.** `rinexpy.ppp.ppp_solve` composes SP3 interpolation,
...
- **SSR composer.** `rinexpy.ssr.SSRCorrections` absorbs decoded RTCM3
...
- RINEX 3 OBS and NAV. Rewritten to drop the O(N²)
  `xarray.merge`-per-epoch pattern from upstream. Headline speedup on
...
- GPT2w empirical surface-meteorology and VMF1-coefficient grid reader
  and evaluator. Grid file is user-supplied, not shipped.
